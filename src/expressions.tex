...
in terms of its member function template call operator.
...
operator member template that is a function template whose
...
struct Fun {
  template<C T> auto operator()(T& a, T* b) const { a = *b; }
} fun;
\end{codeblock}
\tcode{C} is a 

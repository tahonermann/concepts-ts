...
parameters as described in \ref{dcl.spec.auto.deduct}.
\exitnote
%
Placeholders are also used to signify that a lambda is a generic lambda 
(\ref{expr.prim.lambda}), or that a
...
Modify paragraph 4 in the \Cpp Standard (paragraph 6, here) to allow multiple
...
\begin{addedblock}
\item
for a parameter of a generic \grammarterm{lambda-expression} declared with
a type that contains placeholders, \tcode{T} is the declared type of the
parameter and \tcode{e} is the corresponding argument in an invocation of
the generic lambda's function call operator template;
\item
for an argument deduction constraint (\ref{temp.constr.deduct}),
\tcode{T} is the \grammarterm{trailing-return-type}, and \tcode{e} is the
\grammarterm{expression} of the corresponding
...
\setcounter{Paras}{3}
\pnum
If the \removed{placeholder is}\added{placeholders include} the
...
argument deduction.
%
\added{If \tcode{T} corresponds to the type of a parameter of a
generic \grammarterm{lambda-expression}, then template argument deduction is
performed for all parameter types containing placeholders together.}
% 
...
%
\begin{addedblock}
Obtain a type \tcode{P} from \tcode{T} by replacing each placeholder as follows:
\begin{itemize}
...
\item otherwise, if the placeholder is designated by the \tcode{auto}
type-specifier, replace the occurrence with a new invented type
\grammarterm{template-parameter} or, if \tcode{T} corresponds to a
\grammarterm{function parameter pack}, with a new invented type
\grammarterm{template parameter pack}.

\item otherwise, the placeholder is designated by a
\grammarterm{constrained-type-specifier}. If a placeholder in \tcode{T} or,
for parameters of generic \grammarterm{lambda-expression}s, any preceding
parameter, has already been replaced by an invented
\grammarterm{constrained-parameter} (\ref{temp.param}) with
\grammarterm{constraint-expression}s (\ref{temp.param}) equivalent according
to the rules for comparing expressions in \ref{temp.over.link} to a
\grammarterm{constrained-parameter} whose \grammarterm{qualified-concept-name}
is that of the \grammarterm{constrained-type-specifier}, then replace the
occurrence with the existing invented \grammarterm{constrained-parameter}.
Otherwise, replace the occurrence with a new invented
\grammarterm{constrained-parameter} whose \grammarterm{qualified-concept-name}
is that of the \grammarterm{constrained-type-specifier}.
\end{itemize}
\end{addedblock}
%
Deduce a value for \removed{\tcode{U}}\added{each
invented type template parameter} using the rules
...
\added{
\enternote
multiple function template parameters will be present when deducing types for
generic \grammarterm{lambda-expression}s with multiple parameters with types
...
auto gl2 = [](C a, C b) {};
\end{codeblock}
The types of \tcode{a} and \tcode{b} are the same invented template type
parameter.
\begin{codeblock}
auto gl3 = [](C& a, C* b) {};
\end{codeblock}
The type of \tcode{a} is a reference to an invented template type parameter
\tcode{T}, and the type of \tcode{b} is a \grammarterm{pointer} to \tcode{T}.
\begin{codeblock}
auto gl4 = [](N::C a, C b) {};
auto gl5 = [](D<0> a, D<1> b) {};
\end{codeblock}
In both \grammarterm{lambda-expression}s, the parameters \tcode{a} and
...
\end{codeblock}
The types of \tcode{head} and \tcode{tail} are different. Their respective
introduced \grammarterm{constraint-expression}s are \tcode{C<T>} and
\tcode{(C<U> \&\& ...)}, where \tcode{T} is the template parameter invented
...
C1 f1();              // \tcode{C1} designates a placeholder type
Array<auto, C2> f2(); // \tcode{C2} designates a placeholder for an integer value
Stack<auto, C3> f3(); // \tcode{C3} designates a placeholder for a class template
\end{codeblock}
\exitexample
...
template<C T> void f(T); // OK: the set of concepts referred to by C includes both \#1 and \#2;
                         // concept resolution (\ref{temp.constr.resolve}) selects \#1.
template<D T> void g(T); // OK: the concept-name \tcode{D} refers only to \#3
\end{codeblock}
\exitexample
...
template<Seq<3> T> void f1(T); // OK
template<Seq<> T> void f2(T);  // OK
\end{codeblock}
\exitexample
...
are described in \ref{dcl.spec.auto.deduct}.
% 
